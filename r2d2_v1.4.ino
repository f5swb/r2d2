...
//// Encodeur rotatif memorisation du temps pour eviter des erreurs de changements d'etat 
...
/// Encodeur rotatif 
  ...
  if( etatA != dernierEtatA ){ 
     if( abs(millis() - tempsA) > 150 ){
          if(digitalRead(pinB) != dernierEtatA){
          hourupg++;
          if(hourupg>23)hourupg=0;
          rtc.adjust(DateTime(yearupg, monthupg, dayupg, hourupg, now.minute(), secupg));
          TM1637.printTime(hourupg, minupg, true);
      }
      else{
        minupg++;
         ...
         secupg=00;
         rtc.adjust(DateTime(yearupg, monthupg, dayupg, now.hour(), minupg, secupg));
         TM1637.printTime(hourupg, minupg, true);
      }
      tempsA = millis();
    }
