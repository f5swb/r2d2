/* R2D2 - Arduino nano @F5SWB 2023 / Version 1.3 - 17/04/2023
...
//// Détection PIR SENSOR 
 /*  
    if (digitalRead(PIN_TO_SENSOR) == HIGH) {   
        ...
        blink();
        delay(10000);
        Serial.print("Nb detect avant condition = "); Serial.println(nb_detect); 
        Serial.print("minupg : "); Serial.println(minupg);             
        if(minupg >28 && minupg <32 ||  minupg >58 && minupg <2) {
          Serial.print("minupg > 28 ou 58 : "); Serial.println(minupg);
          nb_detect=0;
          Serial.print("Nb detect après condition pas de mp3 = "); Serial.println(nb_detect);   
        }
        if(nb_detect>=4) {
        Serial.print("Nb detect = "); Serial.println(nb_detect);
        player.play(random(31,45));            
